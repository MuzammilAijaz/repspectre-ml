...
class TestLoad(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_path = os.path.join(self.base_dir, "test")
        ...
        # Create dummy CSV data (2 columns as per current DIMENSION=2)
        data = np.random.rand(10, 2)
        df = pd.DataFrame(data, columns=["ax", "ay"])

        df.to_csv(os.path.join(self.train_path, "barbell", "data1.csv"), index=False)
        df.to_csv(os.path.join(self.valid_path, "barbell", "data1.csv"), index=False)
        df.to_csv(os.path.join(self.test_path, "barbell", "data1.csv"), index=False)
        df.to_csv(os.path.join(self.train_path, "none", "data1.csv"), index=False)
