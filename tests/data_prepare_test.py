...
from lift_ml.data.prepare_dataset import organize_sql_data, save_detected_reps_to_csv
...
class TestPrepareDataset(unittest.TestCase):
    ...
    def test_save_detected_reps_to_csv_extracts_and_saves_files(self) -> None:
        ...
        save_detected_reps_to_csv(sessions, output_folder, "prefix", fs=float(fs))
    ...
    def test_save_detected_reps_to_csv_skips_failed_detections(self) -> None:
    ...
    def test_organize_sql_data_integration(self) -> None:
        db_path = os.path.join(self.test_dir, "test.db")
        conn = sqlite3.connect(db_path)

        # Create tables
        conn.execute(
            "CREATE TABLE session (sessionId INTEGER PRIMARY KEY, liftCategory TEXT, noise TEXT);"
        )
        conn.execute(
            "CREATE TABLE rawdata (sessionId INTEGER, ax REAL, ay REAL, az REAL, gx REAL, gy REAL,"
            " gz REAL);"
        )

        # Add a barbell session
        conn.execute(
            "INSERT INTO session (sessionId, liftCategory, noise) VALUES (1, 'FLOOR_PULL', NULL);"
        )
        ...
        for i in range(samples):
            ...
            conn.execute(
                "INSERT INTO rawdata (sessionId, ax, ay, az, gx, gy, gz) VALUES (1, 0, ?, 0, 0, 0,"
                " 0);",
                (val,),
            )
        ...
        # Add a noise session
        conn.execute(
            "INSERT INTO session (sessionId, liftCategory, noise) VALUES (2, NULL, 'TALKING');"
        )
