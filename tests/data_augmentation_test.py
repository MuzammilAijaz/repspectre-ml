...
# pyright: reportUnknownMemberType=false, reportUnknownVariableType=false, reportUnknownArgumentType=false, reportMissingTypeStubs=false, reportPrivateUsage=false
...
from lift_ml.data.augment import _time_wrapping, augment_data
...
class TestAugmentation(unittest.TestCase):
    ...
    def test_time_wrapping_preserves_feature_dimensions(self) -> None:
        original_data = cast_to_list(np.random.rand(10, 3).tolist())
    ...
    def test_time_wrapping_returns_none_when_sequence_too_short(self) -> None:
        data = cast_to_list(np.random.rand(3, 20).tolist())
        self.assertIsNone(_time_wrapping(4, 5, data))

    def test_augment_data_returns_expected_number_of_samples(self) -> None:
        original_data = [
    ...
    def test_augment_data_short_sequence_returns_fewer_than_full_count(self) -> None:
    ...
    def test_augment_data_mixed_lengths_is_not_multiple_of_full_count(self) -> None:
        original_data = [
...
def cast_to_list(val: object) -> list[list[float]]:
    return val  # pyright: ignore[reportUnknownVariableType, reportReturnType]
